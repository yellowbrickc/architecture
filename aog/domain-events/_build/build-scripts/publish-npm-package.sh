...
main() {
  tag="${PUBLISH_TAG:-latestbuild}"
  print_info "publishing npm package with version: '${VERSION}' and tag '${tag}'"

  packagejson_path=$(find $(pwd) -name package.json -type f -not -path */node_modules/* | tail -n1)

  if [[ ! -e "${packagejson_path}" ]]; then
   assert_error "unable to retrieve path of package.json"
  fi

  cd $(dirname "${packagejson_path}")

  npm --no-git-tag-version version "${VERSION}"
  exit_code=$?
  if [[ ${exit_code} -ne 0 ]]; then
    print_warn "unable to change version at package.json (exit code: ${exit_code}). File might already contain ${VERSION}."
  fi

  npm publish --tag "${tag}"
  assert_exit_code "npm publish with tag ${tag} failed"
}
